...
const Step4: React.FC<Step4Props> = ({ 
  ...
  const downloadTranscription = () => {
    const content = `TRANSCRIPTION - ${new Date().toLocaleDateString()}
...
`).join('\n')}`;
    ...
    a.href = url;
    a.download = 'transcription-and-key-points.txt';
    a.click();
    URL.revokeObjectURL(url);
  };

  const downloadTranscriptionOnly = () => {
    const content = `AUDIO TRANSCRIPTION - ${new Date().toLocaleDateString()}
...
---
Generated by Rekapp - Professional Content Creation Platform`;

    const blob = new Blob([content], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'audio-transcription.txt';
    a.click();
    URL.revokeObjectURL(url);
  };

  // Check if this is from audio processing (not text input)
  const isFromAudioProcessing = appState.audioFile || appState.audioUrl || appState.youtubeUrl;
