    ...
    <div className="container mx-auto px-4 py-12 relative z-10">
      <header className="text-center mb-12">
        <div className="inline-block relative mb-6">
          <div className="relative flex items-center justify-center bg-card rounded-full p-4 border-2 border-primary">
            <TrendingUp className="w-12 h-12 text-primary" />
          </div>
        </div>
        <h1 className="text-5xl font-bold text-gray-100 mb-3 font-headline">
          VibeStr Profit
        </h1>
        <p className="text-lg text-gray-400 max-w-2xl mx-auto">
          Your magical calculator for{" "}
          <span className="font-semibold text-gray-200">$VIBESTR</span> profit
          strategies with 10% fee calculations
        </p>
        <div className="mt-6 inline-flex items-center bg-card backdrop-blur-md px-5 py-2 rounded-full border border-border">
          {isPriceLoading ? (
             <div className="flex items-center space-x-2">
                <div className="w-3 h-3 rounded-full bg-primary animate-pulse"></div>
                <span className="text-gray-300 text-sm">Fetching ETH price...</span>
             </div>
          ) : (
            <div className="flex items-center space-x-2">
                
                <span className="text-green-400 font-mono text-sm">Eth = {ethPrice?.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}</span>
            </div>
          )}
        </div>
      </header>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="max-w-4xl mx-auto">
        <TabsList className="grid w-full grid-cols-2 bg-card p-1 h-auto">
          <TabsTrigger value="calculator" className="gap-2 data-[state=active]:bg-muted data-[state=active]:text-foreground">
            <Calculator className="w-5 h-5" /> Calculator
          </TabsTrigger>
          <TabsTrigger value="history" className="gap-2 data-[state=active]:bg-muted data-[state=active]:text-foreground">
            <Clock className="w-5 h-5" /> History ({history.length})
          </TabsTrigger>
        </TabsList>

        <Card className="bg-card/80 backdrop-blur-xl border-border shadow-2xl mt-1 shadow-primary/5">
            ...
            <CardContent className="p-6 md:p-8">
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                        ...
                        <FormItem>
                          ...
                          <FormControl>
                            <div className="relative">
                               <span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-primary font-mono font-bold text-lg">Ξ</span>
                               <Input type="number" step="any" placeholder="0.00" className="pl-8 pr-4 py-3 h-auto bg-input border-border focus:ring-primary" {...field} value={field.value || 0} />
                            </div>
                          </FormControl>
                           {results && <p className="text-xs text-muted-foreground mt-1 h-4">≈ ${results.investmentUsd.toFixed(2)} USD at purchase</p>}
                          <FormMessage />
                        </FormItem>
                      )}
                    />
                     <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>$VIBESTR Tokens Received</FormLabel>
                           <FormControl>
                            <div className="relative">
                              <Coins className="absolute left-3 top-1/2 transform -translate-y-1/2 text-primary w-4 h-4"/>
                              <Input type="number" step="any" placeholder="0.00" className="pl-10 pr-4 py-3 h-auto bg-input border-border focus:ring-primary" {...field} value={field.value || 0} />
                            </div>
                          </FormControl>
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                     <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>$VIBESTR Price at Purchase (USD)</FormLabel>
                           <FormControl>
                            <div className="relative">
                              <DollarSign className="absolute left-3 top-1/2 transform -translate-y-1/2 text-primary w-4 h-4"/>
                              <Input type="number" step="any" placeholder="0.000000" className="pl-10 pr-4 py-3 h-auto bg-input border-border focus:ring-primary" {...field} value={field.value || 0} />
                            </div>
                          </FormControl>
                          <FormMessage className="pt-1"/>
                        </FormItem>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <div className="relative">
                            <Target className="absolute left-3 top-1/2 transform -translate-y-1/2 text-primary w-4 h-4 pointer-events-none"/>
                            <Select onValueChange={field.onChange} value={field.value} defaultValue={"2"}>
                              <FormControl>
                                <SelectTrigger className="pl-10 bg-input border-border focus:ring-primary h-auto py-3">
                            ...
                            <FormField
                              ...
                              render={({ field }) => (
                                <FormItem className="mt-2">
                                  <FormControl>
                                    <Input type="number" step="any" placeholder="Enter custom multiplier" className="bg-input border-border" {...field} value={field.value || 0}/>
                                  </FormControl>
                                  <FormMessage />
                                </FormItem>
                              )}
                  ...
                  <div className="flex justify-center gap-4">
                    <Button type="submit" size="lg" className="px-10 py-4 h-auto bg-primary text-primary-foreground font-bold text-lg hover:bg-primary/90 transition-all transform hover:scale-105">Calculate</Button>
                    <Button type="button" size="lg" onClick={saveTrade} variant="secondary" className="px-10 py-4 h-auto bg-green-500 text-white font-bold text-lg hover:bg-green-600 transition-all transform hover:scale-105">
              ...
              {results && (
                <div className="mt-8 pt-8 border-t border-border/50 space-y-6">
                   <div className="bg-muted/50 p-6 rounded-xl border border-border/20">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-lg font-semibold text-gray-200">Calculated Purchase Price</h3>
                            <div className="px-3 py-1 bg-primary/20 rounded-full text-xs font-medium text-primary">After 10% Fee</div>
                        </div>
                        <div className="grid md:grid-cols-2 gap-4 font-mono">
                            <div>
                                <p className="text-sm text-gray-400 mb-1 font-sans">Per Token (ETH)</p>
                                <p className="text-2xl font-bold">Ξ {results.buyPriceEth.toFixed(8)}</p>
                            </div>
                            <div>
                                <p className="text-sm text-gray-400 mb-1 font-sans">Per Token (USD)</p>
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                         <div className="bg-muted/50 p-6 rounded-xl border border-border/20">
                            <h3 className="text-lg font-semibold text-gray-200 mb-4">Break-even Point</h3>
                            <div className="space-y-3 font-mono">
                                <div>
                                    <p className="text-sm text-gray-400 mb-1 font-sans">Sell Price (ETH)</p>
                                    <p className="text-2xl font-bold">Ξ {results.breakEvenPriceEth.toFixed(8)}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-400 mb-1 font-sans">Sell Price (USD)</p>
             ...
             <CardContent className="p-6 md:p-8 pt-0">
                {history.length === 0 ? (
                    <div className="text-center py-12">
                        ...
                        <h3 className="text-lg text-muted-foreground mt-4">No purchases saved</h3>
                        <p className="text-gray-500 mt-2">Your saved calculations will appear here</p>
                    </div>
                ) : (
                    <div className="space-y-4">
                        {history.map(item => (
                            <div key={item.id} className="bg-muted/50 p-4 rounded-lg border border-border/50">
                                <div className="flex justify-between items-start mb-2">
                                    <div>
                                        <h3 className="font-medium text-foreground">{item.date}</h3>
                                        <p className="text-sm text-muted-foreground font-mono">ETH Price: ${item.ethPrice.toFixed(2)}</p>
                                    </div>
                                ...
                                <div className="grid md:grid-cols-4 gap-4 mt-3 font-mono">
                                    <div>
                                        <p className="text-xs text-primary font-sans">Investment</p>
                                        <p>Ξ {Number(item.investment).toFixed(4)}</p>
                                        <p className="text-xs text-muted-foreground">(${(item.results.investmentUsd).toFixed(2)})</p>
                                    </div>
                                    <div>
                                        <p className="text-xs text-primary font-sans">$VIBESTR</p>
                                        <p>{Number(item.tokens).toFixed(2)}</p>
                                        <p className="text-xs text-muted-foreground">@ ${Number(item.tokenPrice).toFixed(6)}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs text-accent font-sans">Break-even</p>
                                        <p>Ξ {item.results.breakEvenPriceEth.toFixed(8)}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs text-green-400 font-sans">Multiplier</p>
      ...
      <footer className="text-center text-gray-500 text-xs mt-8 space-y-1">
        <div>
          created by <a href="https://x.com/nftmooods" target="_blank" rel="noopener noreferrer" className="hover:text-primary transition-colors">@nftmooods</a>
        </div>
        <div>
